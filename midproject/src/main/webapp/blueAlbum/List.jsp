...
<body>
	...
	<div class="header">
		<h1>
			<a href="../albumBoard/list.do">앨범 게시판</a>
		</h1>
		<c:choose>
			<c:when test="${sessionScope.kind != 2}">
				<button class="btn-write"
					onclick="location.href='../albumBoard/write.do';">글쓰기</button>
			</c:when>
		</c:choose>
				...
				<c:forEach items="${boardList}" var="list" varStatus="stat">
					...
					<td align="center">
						<table>
							...
							<tr>
								<td colspan="4"><a
									href="../albumBoard/view.do?alnum=${list.alnum}">${list.title}
								</a></td>
							</tr>
							<tr>
								<td colspan="4"><img
									src="<%=request.getContextPath()%>/Storage/${list.nfile}"
