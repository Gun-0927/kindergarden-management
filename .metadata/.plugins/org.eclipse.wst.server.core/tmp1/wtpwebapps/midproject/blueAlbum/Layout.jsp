...
<% if(session.getAttribute("id")==null) { //로그인 상태 확인, 로그아웃 된 상태 %> 
    ...
    <aside class="sidebar" style="display: inline-block; width: 200px;">
        <h2>카테고리</h2>
        <a href="../Board/Enterance.jsp" class="category-link <%= (request.getRequestURI().endsWith("Enterance.jsp")) ? "active" : "" %>">홈</a>
        <a href="../Board/List.jsp" class="category-link <%= (request.getRequestURI().endsWith("List.jsp")) ? "active" : "" %>">공지사항</a>
        <a href="../Board/List2.jsp" class="category-link <%= (request.getRequestURI().endsWith("List2.jsp")) ? "active" : "" %>">알림장</a>
        <a href="List.jsp" class="category-link <%= (request.getRequestURI().endsWith("../blueAlbum/List.jsp")) ? "active" : "" %>">앨범</a>
        <a href="Calendar.jsp" class="category-link <%= (request.getRequestURI().endsWith("Calendar.jsp")) ? "active" : "" %>">일정표</a>
        <a href="../Board/List3.jsp" class="category-link <%= (request.getRequestURI().endsWith("List3.jsp")) ? "active" : "" %>">반정보</a>
    </aside>
	<%	}%>
</body>
</html>